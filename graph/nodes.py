...
def output_guard_node(state: ParkingState) -> dict:
    ...
    # Skip the LLM check when collecting reservation fields OR when delivering
    # an admin approval/rejection decision (generated by admin_agent, safe by construction).
    in_reservation = bool(
        state.get("reservation_step") and state.get("reservation_step") != "complete"
    )
    is_approval_decision = state.get("approval_status") in ("approved", "rejected")
    is_safe, reason = check_output(answer, skip_llm_check=(in_reservation or is_approval_decision))
